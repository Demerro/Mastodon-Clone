...
extension ProfileRequest: RequestProtocol {
    ...
    func response() async throws(Error) -> Account {
        Logger.accountsDomain.info("Starting request for current account")
        ...
        do {
            ...
            Logger.accountsDomain.error("Encountered decoding error: \(decodingError)")
            throw .decoding(decodingError)
        } catch let error {
            Logger.accountsDomain.error("Encountered unknown error: \(error)")
            throw .unknown(error)
        }
    }
}
