...
extension UnreblogsStatusRequest: RequestProtocol {
    
    func response() async throws(MastodonError) -> Status {
        Logger.reblogs.debug("Starting request for unreblog status")
        
        var urlComponents = URLComponents()
        urlComponents.scheme = "https"
        urlComponents.host = instanceHost
        urlComponents.path = "/api/v1/statuses/\(id)/unreblog"
        ...
        do {
            ...
            Logger.reblogs.info("Request for unreblog status succeeded")
            return decodedData
...
extension UnreblogsStatusRequest: Sendable {
}
